...
public interface ViewMixin<M, C extends ControllerBase<M>> extends Projector<M, C> {
	 ...
	 * 'Property'
	 */
	default <V> ListConverter<V> onChangeOf(ObservableList<V> observableList) {
		return new ListConverter<>(observableList);
	}
	...
	record Converter<V>(ObservableValue<V> observableValue) {
		 ...
		 *
		 * @param converter
		 * 		the function converting the type of 'ObservableValue' into the type of the 'Property'
		 *
		 ...
		 *
		 * @param property
		 * 		GUI-Property that will be updated when observableValue changes
		 */
		 ...
		 *
		 * @param listener
		 * 		whatever needs to be done on GUI when observableValue changes
		 */
	 ...
	 *
	 */
	record ListConverter<V>(ObservableList<V> observableList) {
		 ...
		 * @return an Updater to specify the 'GUI-Property' that needs to be updated if 'ObservableValue' changes
		 */
		public <R> ListUpdater<V, R> convertedBy(Function<List<V>, List<R>> converter) {
			return new ListUpdater<>(observableList, converter);
		}
		 ...
		 * 		GUI-Property that will be updated when observableValue changes
		 */
		public void update(Property<List<? super V>> property) {
			execute((oldValue, newValue) -> property.setValue(newValue));
		}
		 ...
		 * 		whatever needs to be done on GUI when observableValue changes
		 */
		public void execute(ObservableList.ValueChangeListener<V> listener) {
	...
	record Updater<V, P>(ObservableValue<V> observableValue, Function<V, P> converter) {
		...
		/**
		 * Registers an observer that will keep observableValue and GUI-Property in sync by applying the specified
		 * converter.
		 *
		 * @param property
		 * 		GUI-Property that will be updated when observableValue changes
	...
	record ListUpdater<V, P>(ObservableList<V> observableList, Function<List<V>, List<P>> converter) {
		 ...
		 * 		GUI-Property that will be updated when observableValue changes
		 */
		public void update(javafx.collections.ObservableList<? super P> property) {
			observableList.onChange((oldValue, newValue) -> {
				List<P> convertedValue = converter.apply(newValue);
				Platform.runLater(() -> property.setAll(convertedValue));
			});
		}
	}
