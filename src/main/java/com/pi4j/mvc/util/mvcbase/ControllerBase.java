 ...
 * <p>
 * Execution of Actions is asynchronous. The sequence is kept stable, such that for all actions A and B: if B is
 * submitted after A, B will only be executed after A is finished.
 */
public abstract class ControllerBase<M> {
	 ...
	 * Value is set asynchronously.
	 */
	protected <V> void appendToList(ObservableList<V> observableList, V newValue) {
		async(() -> observableList.append(newValue));
	}
	 ...
	 * Value is set asynchronously.
	 */
	protected <V> void setList(ObservableList<V> observableList, List<V> newValue) {
		async(() -> observableList.setValue(newValue));
	}
